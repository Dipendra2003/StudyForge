...
export class LeaderboardService {
  ...
  async getCategoryLeaderboard(
    ...
    try {
      ...
      const categoryPerformers = await db
        ...
        .from(userQuizStats)
        .innerJoin(users, eq(userQuizStats.userId, users.id))
        .where(sql`JSON_EXTRACT(${userQuizStats.categoryStats}, ${`$.${category}`}) IS NOT NULL`);
  ...
  async getDifficultyLeaderboard(
    ...
    try {
      ...
      const difficultyPerformers = await db
        ...
        .from(userQuizStats)
        .innerJoin(users, eq(userQuizStats.userId, users.id))
        .where(sql`JSON_EXTRACT(${userQuizStats.difficultyStats}, ${`$.${difficulty}`}) IS NOT NULL`);
      ...
      console.error('Error getting difficulty leaderboard:', error);
  ...
  async updateLeaderboard(
    ...
    try {
      ...
      if (stats.length === 0) {
        console.warn(`User stats not found for userId ${userId}. Creating initial record.`);
      ...
      console.log(`Leaderboard updated for user ${userId}: score=${score}, category=${category}, difficulty=${difficulty}`);
    } catch (error) {
